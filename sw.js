...
var assetsToCache = [
  ...
  '/css/typo.css',
  '/css/google-code/desert.css',
  '/sw.js',
  '/js/jquery-3.1.1.js',
  '/css/google-code/prettify.js',
    ...
    evt.respondWith(
      caches.match(evt.request).then(function (response) {
        ...
        return fetch(request).then(function (response) {
          ...
          // 请求结果不是200且静态资源，不缓存
          if (!response || !response.status == 200 || !response.headers.get('Content-type')|| !response.headers.get('Content-type').match(/image|javascript|json|test\/css/i)) {
            return response;
          }
          // 缓存图片等其他get请求
